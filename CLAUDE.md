...
| `command.rs` | `clap` CLI definition and subcommand dispatch |
...
| `pipe.rs` | Stream raw FLV to stdout with reconnection and `FlvStripper` |
| `record.rs` | Record to file — `AudioRecorder` (ffmpeg-next: AAC→WAV/MP3/FLAC) or `DownloadRecorder` (FLV byte filtering), plus FLV trimming and ffmpeg remux for MP4 output |
...
CLI args → resolve_room_id → wait_for_live (poll / 5 s)
         ...
         → [record video] reqwest streaming → FlvFilter → write FLV file → (optional) external ffmpeg remux to MP4
```
...
- **MP4 output** downloads as FLV first (via `DownloadRecorder`), then spawns external `ffmpeg` to remux. A `trim_flv()` step removes trailing garbage from truncated FLV files before remuxing.
...
- Temp FLV files for MP4 recording use `.tmp.flv` extension (also git-ignored).
