...
- **FFmpeg-native encoding** (`AudioRecorder`) runs on a blocking threadpool via `tokio::task::spawn_blocking` because `ffmpeg-next` is synchronous. Decoder and encoder persist across reconnections — only the input stream is reopened. The encoder is therefore flushed exactly once, in `finish()`, never in `record_connection()`: `avcodec_send_frame` returns `AVERROR_EOF` for every frame sent after a flush, so flushing per connection would break reconnection.
- **Audio FIFO** (`AudioFifo`) sits between resampler and encoder. libmp3lame (1152) and flac (4608) reject any frame that is not exactly `frame_size` samples — the first short frame is treated as the last, and the next one fails with `EINVAL` — so resampled frames, whose length follows the input, are re-cut into exact chunks. Samples are buffered as raw bytes, one buffer per plane, which handles packed and planar layouts uniformly. Encoder frames carry an explicit PTS counted in samples.
- **Resampler output frames must be pre-allocated.** `resampling::Context::run` sizes an empty output frame from the *input* sample count, which truncates whenever the output rate is higher than the input rate (44.1 kHz → 48 kHz silently loses ~8%). Allocate `delay().output + ceil(in_samples × out_rate / in_rate)` plus margin, and flush the resampler's own buffer at end of connection.
